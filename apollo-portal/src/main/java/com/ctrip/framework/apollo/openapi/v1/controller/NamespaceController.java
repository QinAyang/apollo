...
public class NamespaceController {
    ...
    /******************     added by pegasus     ****************************/
    @GetMapping(value = "/openapi/v1/envs/{env}/apps/{appId}/clusters/{clusterName}/namespaces/ex")
    public List<OpenNamespaceDTO> findNamespacesEx(@PathVariable final String appId,
                                                   @PathVariable final String env,
                                                   @PathVariable final String clusterName) {
        final List<OpenNamespaceDTO> result = this.findNamespaces(appId, env, clusterName);
        final RsaKey rsaKey = this.namespaceService.getRsaKey(env, clusterName);
        if (null == rsaKey) {
            return result;
        }
        for (final OpenNamespaceDTO openNamespaceDTO : result) {
            for (final OpenItemDTO item : openNamespaceDTO.getItems()) {
                item.setValue(EncryptUtil.decrypt(item.getValue(), rsaKey, new ArrayList<>()));
            }
        }
        return result;
    }
