...
public class NamespaceService {
    ...
    /******************     added by pegasus     ****************************/
    public RsaKey getRsaKey(final String env,
                            final String clusterName) {
        final List<NamespaceBO> encryptNamespace = this.findNamespaceBOs(this.apolloEncryptAppId, Env.valueOf(env), clusterName);
        if (null == encryptNamespace || encryptNamespace.isEmpty()) {
            return null;
        }

        final RsaKey rsaKey = new RsaKey();
        for (final NamespaceBO namespaceBO : encryptNamespace) {
            for (final ItemBO item : namespaceBO.getItems()) {
                if (this.apolloEncryptPublicKeyName.equals(item.getItem().getKey())) {
                    rsaKey.setPublicKey(item.getItem().getValue().trim());
                } else if (this.apolloEncryptPrivateKeyName.equals(item.getItem().getKey())) {
                    rsaKey.setPrivateKey(item.getItem().getValue().trim());
                }
            }
        }

        if (StringUtils.isBlank(rsaKey.getPublicKey()) || StringUtils.isBlank(rsaKey.getPrivateKey())) {
            return null;
        }
    ...
    private NamespaceBO transformNamespace2BO(Env env, NamespaceDTO namespace) {
        ...
        //deleted items
        itemService.findDeletedItems(appId, env, clusterName, namespaceName).forEach(item -> deletedItemDTOs.put(item.getKey(), item));
