...
public class ConfigController {
    ...
    public ApolloConfig queryConfig(@PathVariable String appId, @PathVariable String clusterName,
        ...
        /******************     added by pegasus     ************************/
        final Release encryptRelease = this.configService.loadConfig(this.apolloEncryptAppId.trim(),
                clientIp,
                this.apolloEncryptAppId.trim(),
                ConfigConsts.CLUSTER_NAME_DEFAULT,
                ConfigConsts.NAMESPACE_APPLICATION,
                dataCenter,
                null);
        if (null != encryptRelease) {
            final Map<String, String> encryptMap = mergeReleaseConfigurations(Collections.singletonList(encryptRelease));
            final RsaKey rsaKey = new RsaKey();
            rsaKey.setPrivateKey(encryptMap.get(this.apolloEncryptPrivateKeyName.trim()));
            rsaKey.setPublicKey(encryptMap.get(this.apolloEncryptPublicKeyName.trim()));
            if (!StringUtils.isBlank(rsaKey.getPublicKey()) && !StringUtils.isBlank(rsaKey.getPrivateKey())) {
                EncryptUtil.decrypt(apolloConfig.getConfigurations(), rsaKey, new ArrayList<>());
            }
        }
        /******************     added by pegasus     ****************************/
